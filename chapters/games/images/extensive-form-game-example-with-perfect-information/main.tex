...
\begin{tikzpicture}
    \node [draw, color=gtbcoral] (A1) at (0, 0) {\(A_1\)};
        \node [draw, color=gtbblue] (B1) at ($(A1) + (-2.5, -2)$) {\(B_1\)};
            \node [draw] (O1) at ($(B1) + (-1, -2)$) {\tiny{\(O_1\)}};
            \node (SS) at ($(O1) + (0, -.5)$) {\((4, 1)\)};
            \node [draw] (O2) at ($(B1) + (1, -2)$) {\tiny{\(O_2\)}};
            \node (SC) at ($(O2) + (0, -.5)$) {\((3, 5)\)};
        \node [draw, color=gtbblue] (B2) at ($(A1) + (2.5, -2)$) {\(B_2\)};
            \node [draw, color=gtbcoral] (A2) at ($(B2) + (-1, -2)$) {\(A_2\)};
