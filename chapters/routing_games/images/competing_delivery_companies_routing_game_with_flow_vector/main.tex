...
\begin{tikzpicture}[sloped]
    ...
    \draw [->] (m1) -- node [above] {\tiny{$c(x)=x$}} node [below,red] {\tiny{$1-\alpha-\beta$}} (t);
    \draw (s2) edge[out=-45,in=-135,->] node [below] {\tiny{$c(x)=\frac{3}{2}x$}} node [above,red] {\tiny$\beta$} (t);
    \draw (s1) edge[out=45,in=135,->] node [above] {\tiny{$c(x)=x^2$}} node [below,red] {\tiny$\alpha$} (t);
