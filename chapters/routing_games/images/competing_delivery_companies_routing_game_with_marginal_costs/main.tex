...
\begin{tikzpicture}[sloped]
    ...
    \draw [->] (m1) -- node [above] {\tiny{$c(x)=2x$}} node [below,gtbcoral] {\tiny{$1-\alpha-\beta$}} (t);
    \draw (s2) edge[out=-45,in=-135,->] node [below] {\tiny{$c(x)=3x$}} node [above,gtbcoral] {\tiny$\beta$} (t);
    \draw (s1) edge[out=45,in=135,->] node [above] {\tiny{$c(x)=3x^2$}} node [below,gtbcoral] {\tiny$\alpha$} (t);
